...
--
-- DESCRIÇÃO:
--   Procedure incremental principal que transforma tb_vendas (raw) em camada
--   semantica completa (trusted -> semantic -> mart), pronta para consumo
--   pelo E-comm Genie via Slack.
--
-- OTIMIZAÇÕES:
--   - MERGE e DELETE em tabelas de grande volume para evitar leituras desnecessárias.
--
-- =============================================================================

CREATE OR REPLACE PROCEDURE ecomm_genie.procedures.ecomm_genie_semantic_incremental()
BEGIN
...
USING (
    ...
    WHERE 1=1 
        ...
        AND DATE(dt_venda) >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
    ) s
ON  t.cod_pedido    = s.cod_pedido
AND t.cod_material  = s.cod_material

WHEN MATCHED AND (
       t.status_oms            <> s.status_oms
    OR t.flg_faturada          <> s.flg_faturada
    OR t.flg_aprovada          <> s.flg_aprovada
    OR t.vlr_receita_faturada  <> s.vlr_receita_faturada
    OR t.vlr_venda_pago        <> s.vlr_venda_pago
    OR t.vlr_venda_desconto    <> s.vlr_venda_desconto
)
THEN UPDATE SET (
    status_oms               = s.status_oms,
    flg_faturada             = s.flg_faturada,
    flg_aprovada             = s.flg_aprovada,
    flg_valido               = s.flg_valido,
    vlr_receita_faturada     = s.vlr_receita_faturada,
    vlr_venda_pago           = s.vlr_venda_pago,
    vlr_venda_desconto       = s.vlr_venda_desconto,
    cpf_hash                 = s.cpf_hash,
...
VALUES (
    ...
    s.cod_material,
    s.cod_material_pai,
    s.apresentacao_combo,
    s.dt_venda,
    s.dt_hora_venda,
    s.cod_ciclo,
    s.des_ciclo,
    s.marca_ind,
    s.categoria_final_nivel1,
    s.regiao,
    s.uf,
    s.des_cidade,
    s.des_canal_venda_final,
    s.fonte_de_trafego_nivel_1,
    s.des_midia_canal,
    s.des_cupom,
    s.flg_pedidos_cd,
    s.flg_pedidos_pickup,
    s.status_oms,
    s.flg_faturada,
    s.flg_aprovada,
    s.flg_valido,
    s.vlr_receita_faturada,
    s.vlr_venda_pago,
    s.vlr_venda_desconto,
    s.cpf_hash,
    CURRENT_TIMESTAMP()
)
;
...
MERGE ecomm_genie.semantic.fato_vendas f
USING (
    SELECT
...
ON  f.cod_pedido    = t.cod_pedido
AND f.cod_material  = t.cod_material

WHEN MATCHED AND (
       f.status_oms         <>  t.status_oms
    OR f.receita_liquida    <>  t.receita_liquida
    OR f.receita_bruta      <>  t.receita_bruta
    OR f.desconto           <>  t.desconto
    OR f.receita_faturada   <>  t.receita_faturada
    OR f.flg_faturada       <>  t.flg_faturada
    OR f.flg_aprovada       <>  t.flg_aprovada
    OR f.flg_cancelado      <>  t.flg_cancelado
)
THEN UPDATE SET (
    timestamp_atualizacao   =   t.timestamp_atualizacao,
    status_oms              =   t.status_oms,
    receita_liquida         =   t.receita_liquida,
    receita_bruta           =   t.receita_bruta,
    desconto                =   t.desconto,
    receita_faturada        =   t.receita_faturada,
    flg_faturada            =   t.flg_faturada,
    flg_aprovada            =   t.flg_aprovada,
    flg_valido              =   t.flg_valido,
    flg_cancelado           =   t.flg_cancelado
)
...
--
-- Grain: 1 linha = dia + marca + categoria + região.
-- Particionada por dt_venda para eficiência em filtros temporais.
-- =============================================================================

DELETE FROM ecomm_genie.mart.vendas_diarias
WHERE dt_venda >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
;

INSERT INTO ecomm_genie.mart.vendas_diarias
SELECT
    f.dt_venda                                                  AS dt_venda,
    t.ano                                                       AS ano,
    t.mes                                                       AS mes,
...
WHERE 1=1 
    AND f.dt_venda >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
GROUP BY 
    f.dt_venda, t.ano, t.mes, t.nome_mes, t.dia_semana, t.trimestre, t.ano_mes,
